...
per-conv_type breakdown, DDP design, --lr CLI arg -- is identical to
...
for the full rationale behind those pieces. This file is a fully separate
script (not an edit to the original) so the original's already-running
training job/checkpoint under its own run_output_dir is left untouched.

Usage (single node, 4 GPUs):
    torchrun --nproc_per_node=4 dev/limix_backbone_gnn_pooling_multi_dataset_multi_view_pooler_multi_task_fit_test.py \\
...
OUTPUT_ROOT = PAPER_DIR / "dev" / "output" / "limix_backbone_gnn_pooling_multi_dataset_multi_view_pooler_multi_task_fit_test"
