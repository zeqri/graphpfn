"""Same frozen-LimiX-16M-backbone + trainable PoolingGNN setup as
limix_backbone_gnn_pooling_fit_test.py, but trained the way the real
pretraining pipeline (bin/graphpfn/pretrain.py) actually trains: a BRAND NEW
synthetic dataset is sampled from the prior for every micro-step, several
micro-steps are gradient-accumulated into one optimizer update (matching
exp/graphpfn/pretrain/main/pretrain.toml's n_gradient_accumulation_steps=20),
and "held-out R2" is measured on genuinely unseen, fixed-seed synthetic draws
(EVAL_SEED_BASE+) rather than the same split the loss was computed on.

This is the key difference from the single-dataset script: there, one
dataset was sampled ONCE (fixed SEED) and the loss/eval R2 were both computed
against that same fixed query split every epoch -- fitting it well over 1000
epochs is basically just full-batch gradient descent converging on one
stationary target, not evidence PoolingGNN generalizes across the prior. Here
PoolingGNN's shared parameters have to work across a constantly-shifting
target (a new dataset -- different n_molecules/n_features/n_groups/graph
structure -- every micro-step), which is a much higher-variance optimization
problem; gradient accumulation over N_GRADIENT_ACCUMULATION_STEPS datasets
per update exists specifically to tame that variance, same as in the real
pipeline.

Because the dataset changes every micro-step, nothing dataset-shaped
(atom_embeddings_grouped, n_features, n_groups, n_molecules, ...) can be
computed once outside the loop anymore -- see sample_and_prepare_dataset,
called fresh every micro-step, unlike the single-dataset script's one-time
encode_atoms_frozen() call.

DDP design: unlike the single-dataset script (same seed on every rank, ranks
split QUERY MOLECULES of one shared dataset), there is no single shared
dataset to split here, so each rank seeds independently (SEED + rank) and
samples its OWN datasets every micro-step -- ordinary data-parallel training,
gradients on PoolingGNN's shared parameters averaged across ranks via
DistributedDataParallel after each optimizer step's accumulated backward
calls (with `no_sync()` on every micro-step but the last, exactly like
bin/graphpfn/pretrain.py's step loop). Held-out eval uses fixed global seeds
(not rank-dependent) and only runs on the main process, since it isn't
sharded and pooler_ema's parameters are already the same on every rank.

Training datasets are sampled by a background-worker DataLoader
(build_prefetch_loader, --n-sampler-workers workers/rank) so the CPU-bound
prior sampling for micro-step i+1 overlaps with the GPU work of micro-step
i, instead of blocking the training loop -- a smaller-scale version of the
real pipeline's GraphPriorSampler (n_workers/prefetch_factor), without its
batch-padding-across-datasets machinery since we still process one dataset
per micro-step. Held-out eval datasets are still sampled synchronously
(sample_and_prepare_dataset) since they're only needed once every
EVAL_EVERY steps -- not worth prefetching.

--n-molecules controls BASE_PRIOR_CONFIG's own n_graphs (molecules/dataset,
3000 by default there). Bigger datasets give a longer in-context sequence
but cost much more to sample on CPU (~9-12s/dataset at 3000, observed) --
--n-sampler-workers/--prefetch-factor need to scale with it (and with your
job's actual CPU budget: on SLURM, all ranks under one `torchrun` share a
single task's --cpus-per-task, NOT --cpus-per-task per rank, so divide by
--nproc_per_node before picking a worker count).

Checkpointing: every EVAL_EVERY steps (and at the final step), the trained
PoolingGNN + EMA + optimizer + lr_scheduler state is saved to
<run_output_dir>/pooler_checkpoint.pt, and a per-step metrics record (loss,
grad_norm, lr, held_out_r2 when evaluated that step) is appended to
<run_output_dir>/training_log.jsonl, where run_output_dir encodes --n-steps
and --n-molecules so different configs never collide/wrongly auto-resume
into each other. Re-running with the SAME arguments auto-resumes from the
checkpoint if one exists -- but only for continuing an INTERRUPTED run of
that exact config, since the cosine lr_scheduler's decay curve is baked in
relative to --n-steps at construction time; extending the total step budget
needs a fresh run (different --n-steps, hence a different output dir), not a
resume.

Usage (single node, 4 GPUs):
    torchrun --nproc_per_node=4 dev/limix_backbone_gnn_pooling_multi_dataset_fit_test.py \\
        --n-molecules 3000 --n-sampler-workers 11 --prefetch-factor 4
...
OUTPUT_ROOT = PAPER_DIR / "dev" / "output" / "limix_backbone_gnn_pooling_multi_dataset_fit_test"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Molecules per sampled synthetic dataset, i.e. BASE_PRIOR_CONFIG's n_graphs (default: "
        "300, a 10x-cheaper-to-sample override of the base config's 3000). Bigger values give a "
        "longer in-context sequence but cost much more CPU time per sample -- scale "
        "--n-sampler-workers up accordingly.",
    )
    parser.add_argument(
        "--n-sampler-workers",
        type=int,
...
def build_prior_config(n_molecules: int) -> dict:
    """A copy of BASE_PRIOR_CONFIG with n_graphs (molecules/dataset) overridden
    to `n_molecules`. Copied rather than mutated in place since
    limix_encoder_pooling_probe.py (where BASE_PRIOR_CONFIG is defined) and
    limix_backbone_gnn_pooling_fit_test.py (the single-dataset script) both
    import and rely on the original, 3000-molecule version.
    """
    prior_config = copy.deepcopy(BASE_PRIOR_CONFIG)
    prior_values = prior_config["prior"]["values"][0]
    prior_values["graph"]["sampler"]["values"][0]["n_graphs"]["values"] = [n_molecules]
    prior_values["graph"]["n_nodes"]["values"] = [n_molecules]  # cosmetic only -- the multi-graph
    # sampler (lib/graphpfn/prior/graphs/multi_graph.py) actually sizes datasets from n_graphs/
    # base_n_nodes above, not this top-level field, but keeping it in sync avoids a misleading stale
    # value.
    return prior_config


def build_run_paths(n_molecules: int, n_steps: int) -> tuple[Path, Path, Path]:
    """run_output_dir encodes both --n-molecules and --n-steps so different
    configs get separate checkpoints/logs -- otherwise re-running with a
    different --n-molecules would try to auto-resume a checkpoint trained
    (and already fully stepped, per the OLD --n-steps) on a totally different
    dataset-size regime, silently doing zero further training.
    """
    run_output_dir = OUTPUT_ROOT / f"n_molecules_{n_molecules}_n_steps_{n_steps}"
...
class _RawSyntheticDatasetIterable(torch.utils.data.IterableDataset):
    ...
    micro-step i, instead of blocking the training loop like the original
    synchronous sample_and_prepare_dataset call did.
    """
...
def forward_pass(model: nn.Module, dataset: SampledDataset, pooler_module: nn.Module) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    (context labels revealed, query masked to NaN) -- same chain as the
    single-dataset script's forward_pass, just parameterized over `dataset`
    instead of closing over module-level state. Returns
...
def main() -> None:
    ...
    prior_config = build_prior_config(args.n_molecules)
    run_output_dir, pooler_checkpoint_path, training_log_path = build_run_paths(args.n_molecules, args.n_steps)
    ...
    # Unlike the single-dataset script, every rank must draw DIFFERENT
    # training datasets (that's the point of data-parallel training here),
    # so each rank gets its own seed -- no shared-dataset trick needed since
    # there's no single fixed dataset to shard by rank anymore.
    ...
    if is_main:
        ...
        print(
            f"Training on a FRESH synthetic dataset ({args.n_molecules} molecules each) every micro-step "
    ...
    def evaluate_held_out() -> float:
        ...
        EMA weights. Unlike the single-dataset script (where the reported
        "test R2" was computed on the exact split the loss was optimized
        against), this is genuinely unseen data -- a disjoint seed range from
    ...
    for step in iterator:
        ...
        for inner_step in range(N_GRADIENT_ACCUMULATION_STEPS):
            ...
            with pooler.no_sync() if (lib.is_ddp() and not is_last_microstep) else nullcontext():
                ...
                # unlike the old synchronous sample_and_prepare_dataset call.
