"""Extends limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_fit_test.py
with a different fix for the same problem: that script's diagnostic
(dev/diagnose_conv_type_size_r2.py) showed gcn/sage-mean fit very well
(R2 up to 0.9+) while sage-min/sage-max stayed capped around R2<=0.4 and gt
(attention) sat near R2~0 -- uniformly, regardless of molecule count (ruling
out the "min/max becomes less informative on bigger molecules" hypothesis).

Why that gap persisted despite MultiAggregatorConv/MultiAggregatorPool
already computing mean/min/max/attention: both layers COMBINE all four
branches into a single embed_dim vector via one shared, trained Linear layer
(`combine`) before the frozen backbone ever sees the result. That's a real
bottleneck for two separate reasons:
  1. mean/gcn have smooth, dense gradients (every neighbor contributes every
     step); min/max involve a hard, non-differentiable selection (only the
     arg-min/arg-max neighbor gets gradient credit, and WHICH neighbor that
     is can flip step to step) -- a much noisier, slower optimization target
     through the same shared `combine` weights.
  2. gt's label-generating attention (lib/graphpfn/prior/attributes/layers.py's
     GTConv) uses FRESH RANDOM qkv weights for every sampled dataset -- there
     is no single fixed "true attention function" a shared, trained attention
     branch can converge to and reuse; each gt-labeled dataset needs its
     specific random operator INFERRED from context, which is the frozen
     backbone's job (in-context learning), not something a fixed pooler
     branch can precompute once and for all.
Combining before the backbone sees anything asks OUR pooler to solve both
problems by itself. But the frozen backbone already has the right machinery
for exactly this: vendor/limix/model/layer.py's EncoderBaseLayer uses
layer_arch='fmfmsm' == Feature-attention, MLP, Feature-attention, MLP,
Sequence-attention, MLP per layer -- i.e. real multi-head self-attention
ACROSS the "groups" dimension (call_features_attention), not just across
molecules. And add_embeddings (transformer.py) generates its positional
embedding freshly from x's RUNTIME group count on every call (no fixed
max-groups table), so exposing more groups than the model saw at
pretraining is safe.

So this file's PoolingGNN (see MultiViewPool below) stops combining
mean/min/max/attention pooling into one vector at the final atom->molecule
readout. Instead it emits all 4 as SEPARATE groups (4x the group count),
letting the frozen backbone's own cross-group attention decide which view
matters for a given dataset via in-context inference over the revealed
context labels -- rather than asking one shared linear layer to make that
decision once, upfront, identically for every dataset regardless of
conv_type. The per-layer MESSAGE-PASSING step (MultiAggregatorConv, atom<->
atom via real bonds) is left as in the previous script -- expanding THAT
into parallel multi-channel propagation across n_layers would blow up
dimensionality combinatorially (4^n_layers) for comparatively little
expected benefit, since it's the FINAL readout (talking to the frozen
backbone) where the collapse-vs-let-the-backbone-choose distinction actually
matters.

Trade-off: n_groups grows 4x (mean/min/max/attention-pool each becoming
their own group), so the backbone's feature-attention cost (quadratic in
group count) grows roughly ~16x -- feature attention is typically much
cheaper than the sequence/row attention (which scales with n_molecules, up
to thousands), so this is expected to still be a minor fraction of total
step time, but is a real added cost worth watching (see the per-microstep
fetch/encode/fwd/bwd timing already printed during training).
...
(including the per-conv_type/n_molecules breakdown), DDP design, --lr CLI
arg -- is identical to
limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_fit_test.py; see
that file's docstring for the full rationale behind those pieces.
...
OUTPUT_ROOT = PAPER_DIR / "dev" / "output" / "limix_backbone_gnn_pooling_multi_dataset_multi_view_pooler_fit_test"
...
class MultiAggregatorConv(nn.Module):
    ...
    only the FINAL readout (MultiViewPool below) changes in this file; see
    the module docstring for why the message-passing step itself is left
    combining branches while the readout stops doing so.
    """
...
class MultiViewPool(nn.Module):
    ...
    projection (MultiAggregatorPool's approach, see the previous script),
    emits each aggregator's pooled result as its OWN separate group --
    letting the frozen backbone's real cross-group self-attention
    (vendor/limix/model/layer.py's layer_arch='fmfmsm', see this file's
    module docstring) decide which view matters for a given dataset via
    in-context inference over the revealed context labels, instead of
    asking one shared linear layer to make that decision once, upfront, the
    same way for every dataset regardless of conv_type.
    ...
    sage-min/sage-max) and a learned single-head attention pooling (a
    lightweight Set-Transformer-style "pooling by multihead attention"
    readout, complementing MultiAggregatorConv's atom<->atom message-passing
    attention branch with a genuine atom->molecule attention mechanism,
    aimed at gt).
    """
    ...
    def _attention_pool(self, atom_out: torch.Tensor, molecule_id: torch.Tensor, n_molecules: int) -> torch.Tensor:
        ...
        genuine atom->molecule attention, as opposed to MultiAggregatorConv's
        atom<->atom attention over real bonds.
        """
...
class PoolingGNN(nn.Module):
    ...
    same MultiAggregatorConv/LayerNorm instances are called once per group,
    looping over the group dim) -- matches how the real graph adapter
    (GraphPFNGraphAttentionModule) natively operates on the
    (n_nodes, n_groups, embed_dim) shape.

    Unlike the previous script, the final readout (MultiViewPool) is NOT
    collapsed back into one embedding per input group -- it expands each
    group into N_POOL_VIEWS separate groups (mean/min/max/attention-pool),
    so the output has n_groups * N_POOL_VIEWS groups instead of n_groups.
    See this file's module docstring for why (the frozen backbone's own
    cross-group attention can decide which view matters, instead of a
    shared linear layer deciding upfront for every dataset alike).
    """
...
def main() -> None:
    ...
    if is_main:
        ...
        print(
            ...
            f"instead of a combined single embedding -- see module docstring)"
        )
        print(
    ...
    def evaluate_held_out() -> float:
        ...
        docstring. Only ever called from within an `if is_main:` block.
        """
        pooler_ema.eval()
        per_dataset = []
        with torch.no_grad():
