...
Everything else -- the frozen LimiX-16M backbone, PoolingGNN/
MultiAggregatorConv/MultiAggregatorPool (the atom<->atom message-passing +
atom->molecule readout architecture), the widened (min/max-molecules,
conv_type-varying) prior, gradient-accumulated multi-dataset training,
prefetching, checkpointing/resume, per-conv_type breakdown, DDP design,
--lr CLI arg -- is identical to
limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_fit_test.py; see
that file's docstring for the full rationale behind those pieces. This file
is a fully separate script (not an edit to the original) so the original's
already-running training job/checkpoint under its own run_output_dir is
left untouched.

Usage (single node, 4 GPUs):
    torchrun --nproc_per_node=4 dev/limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_multi_task_fit_test.py \\
...
OUTPUT_ROOT = PAPER_DIR / "dev" / "output" / "limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_multi_task_fit_test"
...
class MultiAggregatorPool(nn.Module):
    ...
    per-layer message passing. Unchanged from
    limix_backbone_gnn_pooling_multi_dataset_multi_agg_pooler_fit_test.py.
    """
...
def compute_loss(pred: torch.Tensor, target: torch.Tensor, task_type: TaskType) -> torch.Tensor:
    ...
    ids) -- matches bin/graphpfn/pretrain.py:593-604 exactly (both classification task types share
    the same cross_entropy call; binclass is just 2-class cross-entropy). No slicing needed here
    (unlike _task_metric below): target class ids are always < n_classes <= MAX_N_CLASSES, so the
    unused wider output slots for datasets with fewer classes simply never receive gradient signal.
    """
