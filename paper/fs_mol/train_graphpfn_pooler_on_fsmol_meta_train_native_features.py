...
atom features (the stored node_features / adjacency_lists, no RDKit) -- the training counterpart of
evaluation/eval_graphpfn_pooler_on_fsmol_test_native_features.py.
Usage (single GPU):
    python train_graphpfn_pooler_on_fsmol_meta_train_native_features.py \\
        ...
        [--support-sizes 16,32,64,128] [--max-query 128] \\
...
Usage (4 GPUs, DDP -- same flags, same --output-dir semantics):
    python -m torch.distributed.run --standalone --nproc_per_node=4 train_graphpfn_pooler_on_fsmol_meta_train_native_features.py [...]
    (Slurm: submit_fsmol_meta_train_native_features_ddp4.sbatch)

Smoke-test first with something small and fast, e.g.:
    python train_graphpfn_pooler_on_fsmol_meta_train_native_features.py --n-steps 20 --grad-accum-steps 2 \\
...
if __name__ == "__main__":
    train.main(recipes.NATIVE_FEATURES, __doc__ + "\n" + train.__doc__, output_dir=SCRIPT_DIR / "output" / "fsmol_meta_train_native_features")
