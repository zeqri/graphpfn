"""FS-Mol few-shot eval of **TabICLv2** doing in-context classification directly on FS-Mol's
per-molecule ECFP fingerprints -- NO pooler, NO GNN, NO bond graph, NO RealGraphAdapter.

TabICLv2 sibling of ../limix/eval_limix_backbone_on_fsmol_test_fingerprints.py: the exact same
FS-Mol protocol, task loop, stratified support/query split, feature construction and metric
aggregation, with exactly one thing swapped -- the tabular ICL backend (LimiX classification
retrieval -> TabICLClassifier). Also the FS-Mol analogue of

    tabicl/Test/bace_icl_molebert_tabicl_cls_eval.py

(single-dataset BACE eval) generalised to FS-Mol's 157 held-out few-shot tasks at multiple
support sizes with repeated stratified resamples, instead of one fixed scaffold split.

Per-molecule vector: the 2048-dim ECFP count fingerprint stored in each <task>.jsonl.gz
(`fingerprints`) -- the same feature the FS-Mol Random-Forest baseline uses -- optionally folded
(default 2048 -> 512). `--per-mol-features descriptors` uses the 200-dim RDKit phys-chem
`descriptors` field instead; `both` concatenates folded-fp ++ descriptors.

Protocol matches the FS-Mol paper (fs_mol/utils/test_utils.py::eval_model, Sec 5.1), identical to
the LimiX and pooler eval scripts in this repo:
  ...
  * Per run: clf.fit(X_support, y_support); P_active = clf.predict_proba(X_query)[:, pos_col].
...
TabICLClassifier needs neither CUDA nor a retrieval config; runs on CPU or a single GPU. It reads
<paper>/checkpoints/tabicl/tabicl-classifier-v2-20260212.ckpt (downloaded there on first use if missing;
`--checkpoint-dir` reads it from another directory).
...
Usage:
    python eval_tabiclv2_on_fsmol_test_fingerprints.py \\
        [--fsmol-dir DIR] [--device cuda] \\
        [--per-mol-features fingerprint|descriptors|both] [--fp-fold 512] \\
        [--support-sizes 16,32,64,128,256] [--num-runs 10] [--seed 0] \\
        [--n-estimators 8] [--tabicl-seed 42] [--checkpoint-dir DIR] [--batch-size 8] \\
...
SCRIPT_DIR = Path(__file__).resolve().parent             # <paper>/fs_mol/evaluation/TabICL/
PAPER_DIR = SCRIPT_DIR.parents[2]                        # <paper>/
FSMOL_DIR_DEFAULT = PAPER_DIR.parent / "datasets" / "fs-mol"
CHECKPOINT_DIR_DEFAULT = PAPER_DIR / "checkpoints" / "tabicl"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--seed", type=int, default=0, help="Base seed for the support/query split; run r uses seed+r. Default: 0.")
    p.add_argument("--n-estimators", type=int, default=8, help="TabICL internal ensemble size (TabICLClassifier default).")
    p.add_argument("--tabicl-seed", type=int, default=42, help="random_state passed to TabICLClassifier (deterministic).")
    p.add_argument("--checkpoint-dir", type=Path, default=CHECKPOINT_DIR_DEFAULT,
                   help=f"Dir holding {CLS_CHECKPOINT} (default: <paper>/checkpoints/tabicl; TabICL downloads "
                        "the file there on first use if it is missing).")
    p.add_argument("--batch-size", type=int, default=8, help="TabICLClassifier forward batch size.")
    ...
    p.add_argument("--output-json", type=Path, default=None,
                   help="Default: <script dir>/output/eval_tabiclv2_on_fsmol_test_fingerprints.json "
...
# test_size=None, allow_smaller_test=True)   [copied from the LimiX / pooler eval scripts]
# --------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------
# TabICLv2 tabular in-context classification
# --------------------------------------------------------------------------------------------------

def predict_tabicl(clf, X_sup, y_sup, X_qry):
    """proba[:, pos_col] (P active) for X_qry given (X_sup, y_sup) as context -- fit()/predict_proba(),
    same call pattern as tabicl/Test/bace_icl_molebert_tabicl_cls_eval.py."""
    import numpy as np
    clf.fit(X_sup, y_sup)
    proba = np.asarray(clf.predict_proba(X_qry), dtype=np.float64)
    pos_col = int(np.where(clf.classes_ == 1)[0][0]) if 1 in clf.classes_ else 1
    return proba[:, pos_col]


# --------------------------------------------------------------------------------------------------
# metrics / aggregation   [copied from the LimiX / pooler eval scripts]
# --------------------------------------------------------------------------------------------------
...
def print_report(summary: dict, support_sizes: list[int], tag: str) -> None:
    print(f"\n=== TabICLv2 -> FS-Mol test [{tag}] (tabular ICL on the per-molecule vector, no GNN/pooler) ===")
...
def main() -> None:
    ...
    import numpy as np
    import torch
    sys.path.insert(0, str(PAPER_DIR))          # the vendored TabICL lives in <paper>/vendor/tabicl
    from vendor.tabicl import TabICLClassifier
    ...
    if sharded:
        ...
        print(f"[shard {args.shard_index}/{args.num_shards}] {len(test_task_names)}/{full_count} tasks "
              ...
              f"combine all shards for the paper-comparable number")
    ...
    # ---- predictor (once, reused via fit()/predict_proba() per run -- same TabICLClassifier
    # instance restarted with a fresh context every call, exactly like the BACE reference) --------
    print("Loading TabICLClassifier ...")
    # The vendored TabICL passes the device to torch.cuda.mem_get_info, which rejects a bare "cuda".
    tabicl_device = device
    if tabicl_device.type == "cuda" and tabicl_device.index is None:
        tabicl_device = torch.device("cuda", torch.cuda.current_device())
    clf = TabICLClassifier(
        n_estimators=args.n_estimators,
        batch_size=args.batch_size,
        device=tabicl_device,
        random_state=args.tabicl_seed,
        checkpoint_version=CLS_CHECKPOINT,
        model_path=args.checkpoint_dir / CLS_CHECKPOINT,
        allow_auto_download=True,
        verbose=False,
    )
    checkpoint_desc = str(args.checkpoint_dir / CLS_CHECKPOINT)
    ...
    for ti, name in enumerate(test_task_names):
        ...
        for size in support_sizes:
            ...
            for run_idx in range(args.num_runs):
                ...
                try:
                    pos_prob = predict_tabicl(clf, X_sup, y_sup, X_qry)
    ...
    # ---- save --------------------------------------------------------------------------
    default_name = "eval_tabiclv2_on_fsmol_test_fingerprints.json"
    if sharded:
        default_name = f"eval_tabiclv2_on_fsmol_test_fingerprints.shard{args.shard_index}of{args.num_shards}.json"
    out = args.output_json or (OUTPUT_DIR_DEFAULT / default_name)
    payload = {
        "dataset": "FS-Mol",
        "method": "tabiclv2_tabular_icl",
