"""FS-Mol few-shot eval of **TabPFNv3** doing in-context classification directly on FS-Mol's
per-molecule ECFP fingerprints -- NO pooler, NO GNN, NO bond graph, NO RealGraphAdapter.

TabPFNv3 sibling of ../limix/eval_limix_backbone_on_fsmol_test_fingerprints.py: the exact same
...
retrieval -> TabPFNClassifier.create_default_for_version(ModelVersion.V3)). Also the FS-Mol
analogue of

    tabicl/Test_TabPFNv3/bace_icl_molebert_tabpfn_cls_eval.py
...
TabPFNClassifier.create_default_for_version(ModelVersion.V3) is the gated TabPFN-v3 checkpoint
(repo Prior-Labs/tabpfn_3). It is read from <paper>/checkpoints/tabpfnv3/tabpfn-v3-classifier-v3_default.ckpt
(`--checkpoint-dir` reads it from another directory); if missing, tabpfn downloads it there on first use after
a one-time license acceptance. Runs on CPU or a single GPU.
...
Usage:
    python eval_tabpfnv3_on_fsmol_test_fingerprints.py \\
        ...
        [--n-estimators 8] [--tabpfn-seed 42] [--checkpoint-dir DIR] \\
...
# the TabPFN model cache when one is configured, instead of under $HOME. Set BEFORE importing tabpfn.
...
SCRIPT_DIR = Path(__file__).resolve().parent             # <paper>/fs_mol/evaluation/TabPFNv3/
PAPER_DIR = SCRIPT_DIR.parents[2]                        # <paper>/
FSMOL_DIR_DEFAULT = PAPER_DIR.parent / "datasets" / "fs-mol"
CHECKPOINT_DIR_DEFAULT = PAPER_DIR / "checkpoints" / "tabpfnv3"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-estimators", type=int, default=8, help="TabPFN ensemble size (n_estimators forward passes).")
    p.add_argument("--tabpfn-seed", type=int, default=42, help="random_state passed to TabPFNClassifier (deterministic).")
    p.add_argument("--checkpoint-dir", type=Path, default=CHECKPOINT_DIR_DEFAULT,
                   help=f"Dir holding {CLS_CHECKPOINT} (default: <paper>/checkpoints/tabpfnv3; tabpfn downloads "
                        "the file there on first use if it is missing, after a one-time license acceptance).")
    ...
    p.add_argument("--output-json", type=Path, default=None,
                   help="Default: <script dir>/output/eval_tabpfnv3_on_fsmol_test_fingerprints.json "
...
# --------------------------------------------------------------------------------------------------
# TabPFNv3 tabular in-context classification
# --------------------------------------------------------------------------------------------------

def predict_tabpfn(clf, X_sup, y_sup, X_qry):
    """proba[:, pos_col] (P active) for X_qry given (X_sup, y_sup) as context -- fit()/predict_proba(),
    same call pattern as tabicl/Test_TabPFNv3/bace_icl_molebert_tabpfn_cls_eval.py."""
    import numpy as np
    clf.fit(X_sup, y_sup)
...
def print_report(summary: dict, support_sizes: list[int], tag: str) -> None:
    print(f"\n=== TabPFNv3 -> FS-Mol test [{tag}] (tabular ICL on the per-molecule vector, no GNN/pooler) ===")
...
def main() -> None:
    ...
    # ---- predictor (once, reused via fit()/predict_proba() per run -- same TabPFNClassifier
    # instance restarted with a fresh context every call, exactly like the BACE reference) --------
    print("Loading TabPFNClassifier (v3) ...")
    _kw = dict(n_estimators=args.n_estimators, device=str(device),
               random_state=args.tabpfn_seed, ignore_pretraining_limits=True)
    clf = TabPFNClassifier.create_default_for_version(
        ModelVersion.V3, model_path=str(args.checkpoint_dir / CLS_CHECKPOINT), **_kw
    )
    ...
    for ti, name in enumerate(test_task_names):
        ...
        for size in support_sizes:
            ...
            for run_idx in range(args.num_runs):
                ...
                try:
                    pos_prob = predict_tabpfn(clf, X_sup, y_sup, X_qry)
    ...
    # ---- save --------------------------------------------------------------------------
    default_name = "eval_tabpfnv3_on_fsmol_test_fingerprints.json"
    if sharded:
        default_name = f"eval_tabpfnv3_on_fsmol_test_fingerprints.shard{args.shard_index}of{args.num_shards}.json"
    out = args.output_json or (OUTPUT_DIR_DEFAULT / default_name)
    payload = {
        "dataset": "FS-Mol",
        "method": "tabpfnv3_tabular_icl",
